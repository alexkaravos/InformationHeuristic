...
from src.datasets.dataset_wrappers import DoubleAugmentedDataset,find_knn,augmentedNearestNeighborDataset   
from src.models import resnet
from src.utils import transforms
from src.models.clustering import MLP_dict
from src.clustering.training import train_paired_aug
from src.clustering.losses import ScanLoss


@hydra.main(config_path="../../../configs", 
            config_name="experiments/mnistUnbal/mnistUnbal_scan",
            version_base=None)
...
def main(cfg: DictConfig):
    ...
    #get the nearest neighbors of the originals 
    nn_indices = find_knn(originals,100,metric="cosine")
    #print the knn accuracy
    
    knn_labels = (labels[nn_indices.flatten()].reshape(labels.shape[0],nn_indices.shape[1]) == labels.unsqueeze(1)).sum()
    print(f"KNN accuracy: {knn_labels/len(labels)/nn_indices.shape[1]}")

    print(nn_indices.shape)
    print(type(nn_indices))
    representation_dataset = augmentedNearestNeighborDataset(
        X=originals,
        X_aug=augmentations,
        neighbours=nn_indices,
        labels=labels,
        k=1
    )    
    ...
    #save the nn_indices, labels, representations and augmentations

    torch.save(nn_indices, f"nn_indices.pt")
    torch.save(labels, f"labels.pt")
    torch.save(originals, f"originals.pt")
    torch.save(augmentations, f"augmentations.pt")
