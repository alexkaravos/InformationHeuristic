...
@hydra.main(config_path="../../../configs", config_name="experiments/cifar10/cifar10_barlowtwin", version_base=None)
def main(cfg: DictConfig):
    ...
    save_dir = os.path.join(cfg.save_dir, "barlowtwin_{}_{:.2f}".format(cfg.models.arch, cfg.representations.lambda_param*1000))
    os.makedirs(save_dir, exist_ok=True)    
    torch.save(model_args, f"{save_dir}/model_args.pt")
    
    model = ResNetSSL(**model_args)
    ...
    for epoch in range(0,cfg.representations.epochs,10):
        model.train()   
        ...
        model.eval()
        train_dataset, test_dataset, _ = cifar10_dataset(
            data_folder=cfg.datasets.data_folder,
            normalize=True
        )
